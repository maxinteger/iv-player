...
function main({DOM, Video, Render, Navigator}) {
	...
	const vid1 = DOM.select('.vlink').events('click').map( x => ({type: 'switch', vref: x.target.vref}) );
	const control_ = most
		.mergeArray([play_, pause_, Navigator.navigator_])
		.startWith({});

	const video_ = Video.videoPlay_;

	return {
		Render: video_,
		Video: most.merge(control_, Navigator.navigator_),
		DOM: most
